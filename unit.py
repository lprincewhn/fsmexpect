...
class TestSSHHandler(unittest.TestCase):
    ...
    def testNoPasswordFSM(self):
        ...
        end_state = nmclient.FSMState("end")
        ls_state = nmclient.FSMState("command", "ls")
        ls_state.add_next_state('\[vagrant@devtest ~\]\$', end_state)
        su_state = nmclient.FSMState("command", "su - vagrant", False)
